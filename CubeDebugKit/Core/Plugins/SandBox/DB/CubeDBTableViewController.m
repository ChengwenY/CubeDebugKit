...
@interface CubeDBTableViewController ()<UITableViewDelegate , UITableViewDataSource , CubeDBRowTapProtocol>

@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, copy) NSArray *dataAtTable;
@property (nonatomic, strong) CubeDBShowView *showView;


@end

@implementation CubeDBTableViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = [CubeDBManager sharedInstance].tableName;
    
    NSArray *dataAtTable = [[CubeDBManager sharedInstance] dataAtTable];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    CubeDBRowView *headerView = nil;
    if (headerView == nil) {
        headerView = [[CubeDBRowView alloc] init];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CubeDBCell *cell = [tableView dequeueReusableCellWithIdentifier:identifer];
    if (cell == nil) {
        cell = [[CubeDBCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifer];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.rowView.delegate = self;
    }
    
    cell.rowView.type = (indexPath.row % 2 == 0) ? CubeDBRowViewTypeForOne : CubeDBRowViewTypeForTwo;
...
- (void)showText:(NSString *)content{
    ...
    self.showView = [[CubeDBShowView alloc] initWithFrame:self.view.bounds];
