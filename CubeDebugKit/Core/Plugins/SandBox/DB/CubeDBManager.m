...
@interface CubeDBManager ()

@property (nonatomic, strong) NSMutableArray *dataArray;

@end

@implementation CubeDBManager

+ (instancetype)sharedInstance
{
    static CubeDBManager *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        instance = [[CubeDBManager alloc] init];
    });
...
int selectCallback(void *firstValue,int columnCount, char **columnValues, char **columnNames)
{
    ...
    [[[CubeDBManager sharedInstance] dataArray] addObject:dict];
    return 0;
}
