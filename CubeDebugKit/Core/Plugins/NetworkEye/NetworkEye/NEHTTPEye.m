...
NSString * const NENeedDecryptKey = @"NENeedDecrypt";
NSString * const NENeedDecryptWithBase64DecodeKey = @"NENeedDecryptWithBase64";
NSString * const NEDecryptKey = @"NEDecryptKey";
...
@end
static NERespondDataDecryptBlock cube_decryptBlock;
...
{
    cube_decryptBlock = block;
}

+ (NSData *)decryptHttpBody:(NSData *)httpBody
{
    if (cube_decryptBlock)
    {
        return cube_decryptBlock(httpBody);
    }
    return httpBody;
}
...
-(id)responseJSONFromData:(NSData *)data {
    ...
    NSData *decryptyData = data;
    
    if (cube_decryptBlock)
    {
        decryptyData = cube_decryptBlock(decryptyData);
    }
    
    id returnValue = [NSJSONSerialization JSONObjectWithData:decryptyData options:0 error:&error];
    if(error) {
