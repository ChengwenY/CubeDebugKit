...
-(void)setNe_request:(NSURLRequest *)ne_request_new{
    ...
    NSData *httpBodyData = ne_request.HTTPBody;
    
    httpBodyData = [NEHTTPEye decryptHttpBody:httpBodyData];
    if (httpBodyData)
    {
        if (httpBodyData.length>512) {
            self.requestHTTPBody=@"requestHTTPBody too long";
        }else{
            self.requestHTTPBody= [[NSString alloc] initWithData:httpBodyData encoding:NSUTF8StringEncoding];
        }
    ...
    {
        NSDictionary *dict = [ne_request_new allHTTPHeaderFields];
        NSNumber *contentLength = dict[@"Content-Length"];
        if (contentLength && contentLength.integerValue > 4096) {
            return;
        }
        
        uint8_t byteBuffer[4096];
        NSInputStream *stream = ne_request_new.HTTPBodyStream;
        [stream open];
        ...
        [stream close];
        self.requestHTTPBody= stringFromData;
    }
